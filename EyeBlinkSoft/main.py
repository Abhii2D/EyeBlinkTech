...
brightness_value = 80  # Adjust this value to increase/decrease brightness
bright_frame = cv.convertScaleAbs(frame, alpha=1, beta=brightness_value)
...
current_box = 0
def handle_event(Cindex, previous_key_data):
    actions = {
        0: (utils.PINK, utils.YELLOW, None, None),
        1: (utils.PINK, utils.YELLOW, voice_eat, None),
        2: (utils.PINK, utils.YELLOW, voice_water, None),
        3: (utils.PINK, utils.YELLOW, voice_toilet, None),
        4: (utils.PINK, utils.YELLOW, None, None),
        5: (utils.PINK, utils.YELLOW, voice_tv, None),
        6: (utils.PINK, utils.YELLOW, voice_music, None),
        7: (utils.PINK, utils.YELLOW, voice_call, None),
        8: (utils.PINK, utils.YELLOW, voice_dress, None),
        9: (utils.PINK, utils.YELLOW, voice_child, None),
        10: (utils.PINK, utils.YELLOW, voice_book, None),
        11: (utils.PINK, utils.YELLOW, voice_emergency, None),
        12: (utils.PINK, utils.YELLOW, None, 'exit'),
    }

    default_color = (utils.WHITE, utils.YELLOW)
    
    # Get action or default if Cindex not in actions
    color_bg, color_text, voice_action, command = actions.get(Cindex, default_color + (None, None))

    # Display event text
    utils.colorBackgroundText(keyboard, f'Event is: {previous_key_data["label"]}', FONTS, 0.7, (30, 90), 1, color_bg, color_text)
    print(f"Current Name: {previous_key_data['label']}")
    
    # Play sound or exit based on action
    if voice_action:
        voice_action.play()
    if command == 'exit':
        print("Exiting event...")
